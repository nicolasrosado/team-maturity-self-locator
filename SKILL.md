...
At the close, **offer** to hand the team a **Team Maturity Snapshot** they keep (one artifact: a header + Posture + Waypoint + ADR + a usage-and-license footer; nothing is stored, the skill is stateless). The score is a **private waypoint, not a grade**, it exists only to feed the team's *own* next comparison. **Non-negotiables:** (a) **the output opens with the snapshot header (title + what's inside), then MUST lead with the POSTURE header once, before any numbers**, **reproduced *verbatim*** (copy it word-for-word from the block below, **never paraphrase, shorten or reword it**), then the waypoint and the ADR follow; **do not repeat it before each block.** *(Why it's mandatory: a waypoint **is** a comparison artifact, a comparison **to your team's own past self, never to other teams**, and any comparison already invites the ranking prism, so the anti-ranking frame must travel with the numbers, never after them.)* An output (or any exported file) without it is invalid; (b) the ADR is **always a `PROPOSED` draft**, you *propose*, the **team ratifies** (they name the owner, set the review date, and change the status themselves); (c) if a prior waypoint was pasted at the open, add a short **trajectory** read, an arrow per axis (↗ climbed / → held / ↘ slipped) + **time elapsed** + whether the prior move was kept, comparing the team **only to its own past self**; **never** points, badges, rankings, or another team. **(d) Emit each block exactly once**, whether you write it to the saved file or show it in the chat: once you have printed a block's `=== END ... ===` (or `=== END POSTURE ===`) marker you never output that block again; if you find yourself about to repeat any block or line, stop instead.

**Channel display vs saved artifact (this is the anti-repetition measure, keep the on-screen output short).** The **saved `.txt` file is the full artifact** (header + POSTURE verbatim + Waypoint + full ADR + footer): complete and valid per (a), it is the canonical thing that travels, so write it to a file with the export tool when the host has one, rather than dumping it into the chat. **In the chat channel, show only a lean preview, never the whole artifact:** (i) a **one-line frame** first, `A locator, not a grade - an environment, not a verdict; compare only to your own past self. The full Posture, usage/license terms and ADR notes are in the saved .txt.`; (ii) the **Waypoint block** (verbatim, it is the essential continuity data); (iii) the **ADR core** only, its `#` title, `Status`, `## Context`, `## Decision`, `## Why`, `## Consequences`, and the Owner / Review-date blanks; (iv) a **two-line ADR-notes summary** in place of the ADR's closing explainer block: `(Proposed draft, the team ratifies it: name an owner, set the review date, flip the Status. It is an experiment-commitment to TRY and REVIEW, never a score, target or KPI. Full notes in the saved file.)`. **Do NOT print inline** the snapshot header, the full POSTURE block, the ADR's full closing notes, or the usage/license footer, **offer them on request instead**, as **selectable options** (per the hand-back rule above), and **label each by the user's *need*, never by tool jargon** (adapt to the session language). Use plain, need-naming labels, and give each a short **hook (a why-it's-worth-it), not a flat descriptor**, so it invites exploration, such as **"Read the full Posture"** (*how to keep this a conversation you compare only to your own past self, not a grade or a target that dies the way story-point estimates did in water-scrum-fall*) for the POSTURE block, **"Am I allowed to use this with my team, or a client?"** (or *"What can I do with this Snapshot?"*) for the usage/license footer, and **"What's an ADR, and how do I use this one?"** for the ADR's closing explainer notes. **Never** offer them as *"show posture" / "show license" / "show ADR notes"*, those mean nothing to the user and no one will pick them. The one-line frame **must always precede the inline numbers** (never a bare Waypoint), which satisfies (a) on screen while the complete framing lives in the saved file.
...
**(5) Usage & license footer - the FINAL block; end the output (and the exported file) with it. After its last line (the terms URL) you are DONE: never re-emit the ADR, the footer, or any earlier block, and never loop back into a block whose `=== END ... ===` marker you have already printed. The whole Snapshot is printed once, top to bottom; then nothing follows.**
```
...
- **1.2.0**, on-screen output made lean; the full artifact still travels in the saved `.txt`. In the chat the Snapshot now shows a one-line frame + Waypoint + ADR core + a two-line ADR-notes summary; the full POSTURE, the usage/license footer and the ADR's closing notes are written to the file and offered on request via need-named options (never "show posture / show license"). The close hands back a selection picker (not prose), re-offered after each reveal. Also includes an emit-each-block-once guard against a duplicated on-screen Snapshot. Behaviour of the assessment itself is unchanged; the saved file is unchanged and complete.
